...
MAX_DATA_SIZE = 2500  # Giới hạn số lượng dữ liệu tối đa khi lưu trữ
INITIAL_FETCH_SIZE = 1500  # Số lượng nến lần đầu tải về
...
def download_data(token):
    ...
    file_path = os.path.join(download_path, f"{token.lower()}_data.csv")
    # file_path = os.path.join(data_base_path, f"{token.lower()}_price_data.csv")

    # Kiểm tra xem file có tồn tại hay không
    if os.path.exists(file_path):
        # Tính thời gian bắt đầu cho 100 cây nến 1h 
        start_time = int((current_datetime - timedelta(hours=100)).timestamp() * 1000)
        ...
        start_time = int((current_datetime - timedelta(hours=INITIAL_FETCH_SIZE)).timestamp() * 1000)
        end_time = int(current_datetime.timestamp() * 1000)
...
def format_data(token):
    ...
    file_path = os.path.join(path, f"{token.lower()}_data.csv")
...
def train_model(token):
    ...
    # Resample the data to 1 day frequency
    df = price_data.resample('D').mean()
