...
def train_model(token):
    ...
    price_data.rename(columns={'date': 'ds', 'close': 'y'}, inplace=True)
    price_data.set_index("ds", inplace=True)
    df = price_data.resample('10T').mean().reset_index()

    model = Prophet()
    model.fit(df)

    future = model.make_future_dataframe(periods=1, freq='10T')
